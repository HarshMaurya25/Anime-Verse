...
function FriendProfile() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError('')
    fetch(`http://localhost:11111/api/profile/user/profile/get?id=${encodeURIComponent(friendId)}&image=true`, {
      headers: {
  ...
  const handleToggleFollow = async () => {
    ...
    try {
      if (!isFollowing) {
        // Follow
        const res = await fetch(`http://localhost:11111/api/profile/user/${encodeURIComponent(id)}/follow/${encodeURIComponent(friendId)}`, {
          method: 'POST',
          headers: {
