...
function Friends() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError('')
    fetch(`http://localhost:11111/api/search/user/get?page=${pageNumber}&keyword=${encodeURIComponent(searchTerm.trim())}`, {
      method: 'GET',
      ...
      .then(async (res) => {
        if (!res.ok) {
          ...
          throw new Error(parsed.message || 'Unable to fetch users right now.')
        }
        return res.json()
      })
      .then((data) => {
      ...
      .catch((err) => {
        ...
        setError(err.message || 'Unable to fetch users right now.')
        setResults([])
      })
      ...
      <div className="relative z-10 px-4 pb-20 pt-14 sm:px-8 lg:px-12 xl:px-16">
        ...
        <div className="mt-8 space-y-6">
          <div className="flex flex-col gap-3 rounded-3xl border border-white/10 bg-white/5 p-5 shadow-[0_18px_60px_rgba(0,0,0,0.35)] backdrop-blur-2xl">
            <label className="text-sm font-semibold text-white/90">Find friends</label>
            <div className="flex items-center gap-3 rounded-2xl border border-white/15 bg-black/30 px-3 py-2">
              ...
              <input
                ...
                placeholder="Connect to an otaku"
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 p-5 shadow-[0_18px_60px_rgba(0,0,0,0.35)] backdrop-blur-2xl">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-white">Suggested otaku</h3>
              {loading && <span className="text-xs text-slate-200/75">Searching...</span>}
            </div>
            {error && (
              <div className="mt-4">
            ...
            {!error && !loading && searchTerm.trim() && results.length === 0 && <p className="mt-2 text-sm text-slate-200/85">No users found yet.</p>} 
            {!searchTerm.trim() && <p className="mt-2 text-sm text-slate-200/80">Start typing to discover users.</p>}
