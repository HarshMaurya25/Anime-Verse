...
class ContentControllerTest {
    ...
    @Nested
    @DisplayName("DELETE /content/like-dislike/remove Tests")
    class RemoveLikeDislikeTests {

        @Test
        @WithMockUser(username = "testuser")
        @DisplayName("Should remove like/dislike when authenticated")
        void removeLikeDislike_WhenAuthenticated_Removes() throws Exception {
            LikeDislikeRequest request = LikeDislikeRequest.builder()
                    .contentId(contentId)
                    .userId(userId)
                    .build();

            doNothing().when(contentService).removeLikeDislike(any(LikeDislikeRequest.class));

            mockMvc.perform(delete("/content/like-dislike/remove")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(request))
                    .with(csrf()))
                    .andExpect(status().isOk());
