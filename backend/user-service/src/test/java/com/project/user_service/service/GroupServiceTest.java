...
class GroupServiceTest {
    ...
    class GetGroupTest {
        ...
        void testGetGroupFromDataBase() {
            ...
            when(groupRepository.getGroupById(group1.getId())).thenReturn(Optional.of(groupResponseDto));
            ...
            verify(redisService, times(1)).set(eq(RedisMethod.GROUP_ + group1.getId().toString()), any(),
                    anyLong());
            verify(groupRepository).getGroupById(group1.getId());
        }
        ...
        void testGetDisabledGroup() {
            ...
            when(redisService.get(any(), any())).thenReturn(null);
            when(groupRepository.getGroupById(disabledGroup.getId())).thenReturn(Optional.of(dto));
        ...
        void testGetGroupWithImages() {
            ...
            when(redisService.get(any(), any())).thenReturn(null);
            when(groupRepository.getGroupById(groupId)).thenReturn(Optional.of(dto));
            when(groupRepository.findById(groupId)).thenReturn(Optional.of(group));
            when(imageGroupRepository.findById(groupId)).thenReturn(Optional.of(imageGroup));
    ...
    class UpdateBioTest {
        ...
        void testUpdateBioSuccess() {
            ...
            verify(redisService).delete(RedisMethod.GROUP_ + group1.getId().toString());
            verify(kafkaService).updateIntoGroupDatabase(eq(group1.getId()), isNull(), eq("New bio"), isNull(),
                    isNull());
        }
    ...
    class ChangeLeaderTest {
        ...
        void testChangeLeaderSuccess() {
            ...
            GroupResponseDto result = groupService.changeLeader(group1.getId(), otherUser.getId(), user.getId());
        ...
        void testChangeLeaderGroupNotFound() {
            ...
            assertThatThrownBy(() -> groupService.changeLeader(group1.getId(), otherUser.getId(), user.getId()))
                    .isInstanceOf(GroupNotFoundException.class);
        }
        ...
        void testChangeLeaderNotCurrentLeader() {
            ...
            assertThatThrownBy(() -> groupService.changeLeader(group1.getId(), otherUser.getId(), thirdUserId))
                    .isInstanceOf(IllegalArgumentException.class)
        ...
        void testChangeLeaderDisabledGroup() {
            ...
            assertThatThrownBy(() -> groupService.changeLeader(disabledGroup.getId(), otherUser.getId(), user.getId()))
                    .isInstanceOf(IllegalArgumentException.class)
                    .hasMessageContaining("Group is Block");
        }
